...
from pyrosetta.rosetta.core.simple_metrics import PerResidueRealMetric
...
def requires_show(func: Callable[Concatenate[T, P], R]) -> Callable[Concatenate[T, P], R]:
    """
    Call the `show` method if the 'update_viewer' argument parameter is `True`.
    The 'update_viewer' keyword argument must be the last keyword argument
    of the decorated function.
    """

    @wraps(func)
    def wrapper(self: T, *args: P.args, **kwargs: P.kwargs) -> R:
        if kwargs.get("update_viewer", True) and not self._displayed:
...
def apply_metric_to_poses(metric: PerResidueRealMetric, poses: Union[Iterable[Pose], Pose]) -> None:
    _msg = "The 'poses' argument value must be a `Pose` object or an iterable of `Pose` objects"
    if isinstance(poses, collections.abc.Iterable) and not isinstance(poses, Pose):
        for pose in poses:
            if isinstance(pose, Pose):
                ...
                raise ValueError(f"{_msg}. Received: {type(pose)}")
    elif isinstance(poses, Pose):
        with out:
            metric.apply(poses)
    else:
        raise ValueError(f"{_msg}. Received: {type(poses)}")
...
class PoseBase:
    ...
    def add_pose(
        ...
        Returns:
            `None`
        """
        assert isinstance(pose, Pose), f"Object must be of type `Pose`. Received: {type(pose)}"
        if index is None:
        ...
        if update_viewer:
            ...
            self.update_viewer(index=index, model=model, add_objects=True, remove_objects=False)
    ...
    def remove_pdbstring(
        ...
        *,
        ...
        if update_viewer:
            self.update_viewer(index=index, model=model, add_objects=False, remove_objects=True)
    ...
    def update_pose(
        ...
        *,
        ...
        if update_viewer:
            self.update_viewer(index=index, model=None, add_objects=True, remove_objects=True)
    ...
    def update_pdbstrings(
        ...
        *,
        ...
        Returns:
            `None`
        """
        assert isinstance(pdbstrings, list), "The 'pdbstrings' argument must be of type `list`."
    ...
    def overlay(
        ...
        *,
        ...
        if update_viewer:
            self.update_viewer(index=0, model=None, add_objects=True, remove_objects=True)
